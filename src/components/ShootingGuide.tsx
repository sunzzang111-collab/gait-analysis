...
function FootDiagram() {
  return (
    <svg viewBox="0 0 260 140" className="guide__diagram" role="img" aria-label="근접 발 촬영 배치도">
      ...
      <text x="66" y="60" fontSize="11" fill="#6b7280">러닝머신 뒤</text>
      <text x="66" y="76" fontSize="11" fill="#6b7280">정강이~발</text>
      <text x="66" y="92" fontSize="11" fill="#6b7280">크게</text>
    </svg>
  )
}
...
> = {
  ...
  foot: {
    title: '근접 발 촬영 방법 (러닝머신 권장)',
    orientation: '📱 세로로 — 정강이(무릎 아래)부터 발까지 화면을 채우게',
    steps: [
      '러닝머신 위 걷기(또는 제자리 걷기)에서 촬영하세요. 지상에서 뒤에서 근접하면 환자가 몇 걸음 만에 화면을 벗어나 사실상 불가능합니다.',
      '러닝머신 뒤쪽 바닥이나 낮은 삼각대에 카메라를 고정하고, 정강이~발을 향해 각도를 맞춥니다.',
      '자세 인식을 위해 최소 무릎 아래(정강이)까지는 보여야 합니다 — 발만 확대하면 사람을 인식하지 못해 측정이 안 됩니다.',
